...
public class KeywordsExtractor {
    ...
    public static List<CardKeyword> getKeywordsList(String fullText) throws IOException {
        ...
        try {
            ...
            tokenStream = new StopFilter(new ASCIIFoldingFilter(new ClassicFilter(new LowerCaseFilter(stdToken))), CustomStopset.CUSTOM_STOP_WORDS_SET);
            tokenStream.reset();
    ...
    public static List<WeightedKeyword> getTopN (String fullText, int n) throws IOException {
        List<WeightedKeyword> keywords = new ArrayList<>();
        keywords.addAll(KeywordsExtractor.getKeywordsList(fullText).stream().map(WeightedKeyword::new).collect(Collectors.toList()));
        return keywords;
    }
